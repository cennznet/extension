...
const copySync = require('@polkadot/dev/scripts/copySync.cjs');
...
const esmConfig = require('@polkadot/dev/config/babel-config-esm.cjs');
const cjsConfig = require('@polkadot/dev/config/babel-config-cjs.cjs');
  ...
  await babel({
    babelOptions: {
      configFile: type === 'esm'
        ...
        : path.join(__dirname, 'babel.config.cjs')
    },
    cliOptions: {
      extensions: ['.ts', '.tsx'],
      filenames: ['src'],
